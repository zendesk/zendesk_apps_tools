...
describe ZendeskAppsTools::Command do
  ...
  before do
    ...
    allow(@command.cache).to receive(:fetch_cache)
    allow(@command.cache).to receive(:save_cache)
    allow(@command.cache).to receive(:clear_cache)
  ...
  describe '#update' do
    context 'when app id is in cache' do
      it 'uploads a file and puts build api' do
        expect(@command).to receive(:upload).and_return(123)
        allow(@command).to receive(:check_status)
        expect(@command.cache).to receive(:fetch_cache).with('app_id').and_return(456)
